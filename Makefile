...
build_nix_x64: 
	...
	@nix-build -A deezer-desktop --argstr pkgVer "$(PKGVER)" --out-link ./artifacts/x64/nix-result

build_nix_arm64: 
	@echo "Build Nix package"
	@$(PACKAGE_MANAGER) $(PACKAGE_MANAGER_SUBDIR_ARG) $(APP_DIR) run prepare-nix
	@nix-build -A deezer-desktop --argstr pkgVer "$(PKGVER)" --out-link ./artifacts/arm64/nix-result
